...
{
    public partial class frmGNN_DXGN_KBTT : Form
    {

        GiaynhannoBUS gnn_bus = new GiaynhannoBUS();

        private List<string> nguon_thong_tin_chung = new List<string>();

        private List<string> dich_thong_tin_chung = new List<string>();

        private List<string> nguon_gnn = new List<string>();

        private List<string> dich_gnn = new List<string>();

        internal Int32 id_giay_nhan_no = new Int32();
        ...
        public frmGNN_DXGN_KBTT()
        {
            ...
            tabHo_so_vay.SelectedTab = tabGiay_nhan_no;
        }
        ...
        internal void Danh_sach_giay_nhan_no_theo_ma_hd_vay()
        {            
            DataTable DS_GIAY_NHAN_NO_THEO_MA_HD_VAY = GiaynhannoBUS.DS_GIAY_NHAN_NO_THEO_MA_HD_VAY(txtMa_hd_vay.Text);
            if (DS_GIAY_NHAN_NO_THEO_MA_HD_VAY == null || DS_GIAY_NHAN_NO_THEO_MA_HD_VAY.Rows.Count == 0)
            {
                btnCap_nhat.Enabled = false;
                CommonMethods.ClearTextBoxesInTabPage(tabGiay_nhan_no);
                txtNgay_nhan_no.Text = DateTime.Now.ToString("dd/MM/yyyy");
                txtNgay_de_xuat_giai_ngan.Text = DateTime.Now.ToString("dd/MM/yyyy");
                txtSo_giay_nhan_no.Text = txtma_CN.Text + "LDS" + DateTime.Now.Year.ToString();
                btnCap_nhat.Enabled = false;
                btnXoa.Enabled = false;
                btnTao_giay_nhan_no.Enabled = false;
                btnTao_bao_cao_de_xuat_giai_ngan.Enabled = false;
            }
            else
            {
                btnCap_nhat.Enabled = true;
                btnXoa.Enabled = true;
                btnTao_giay_nhan_no.Enabled = true;
                btnTao_bao_cao_de_xuat_giai_ngan.Enabled = true;
                List<string> number_columns = new List<string>();
                number_columns.Add("DU_NO_TRUOC");
                number_columns.Add("SO_TIEN_NHAN_NO");
                number_columns.Add("DU_NO_HIEN_TAI");
                number_columns.Add("SO_TIEN_NHAN_NO_1");
                number_columns.Add("SO_TIEN_NHAN_NO_2");
                number_columns.Add("SO_TIEN_NHAN_NO_3");
                dgvGiay_nhan_no.DataSource = null;
                dgvGiay_nhan_no.Rows.Clear();

                dgvGiay_nhan_no.DataSource = DS_GIAY_NHAN_NO_THEO_MA_HD_VAY;
                ControlFormat.DataGridView_ShowAllColumn(dgvGiay_nhan_no);
                ControlFormat.DataGridViewFormat(dgvGiay_nhan_no);
                ControlFormat.DataGridView_RemoveEmptyColumns(dgvGiay_nhan_no);
                ControlFormat.DataGridView_NumberColumnFormat(dgvGiay_nhan_no, number_columns);
                dgvGiay_nhan_no.Rows[dgvGiay_nhan_no.Rows.Count -1].Selected = true;
            }
        }
        internal void Cap_nhat_txtMa_hd_vay()
        {
            ...
            //Lấy danh sách giấy nhận nợ theo mã hợp đồng vay
            DataTable DS_GIAY_NHAN_NO_THEO_MA_HD_VAY = GiaynhannoBUS.DS_GIAY_NHAN_NO_THEO_MA_HD_VAY(txtMa_hd_vay.Text);
            if (DS_GIAY_NHAN_NO_THEO_MA_HD_VAY == null || DS_GIAY_NHAN_NO_THEO_MA_HD_VAY.Rows.Count == 0)
            {               
                btnCap_nhat.Enabled = false;
                ...
                txtNgay_de_xuat_giai_ngan.Text = DateTime.Now.ToString("dd/MM/yyyy");
                txtSo_giay_nhan_no.Text = txtma_CN.Text + "LDS" + DateTime.Now.Year.ToString();
                rbHoa_don_mua_hang.Checked = true;
                rbKhac.Checked = false;
                txtChung_tu_giai_ngan_khac.Enabled = false;
                txtChung_tu_giai_ngan_khac.Text = "Khách hàng cam kết cung cấp hóa đơn trước ngày";
                btnCap_nhat.Enabled = false;
                btnXoa.Enabled = false;
                btnTao_giay_nhan_no.Enabled = false;
                btnTao_bao_cao_de_xuat_giai_ngan.Enabled = false;
        ...
        internal void Lay_thong_tin_ho_so_vay()
        {
            ...
            this.nguon_thong_tin_chung.Add("<CHI_NHANH_TEN_CN_VIET_HOA>");
            this.dich_thong_tin_chung.Add(chi_nhanh.ten_CN.ToUpper());
            ...
            {
                this.nguon_thong_tin_chung.Add("<DIA_BAN>");
                this.dich_thong_tin_chung.Add("Thành phố Hải Dương");

                this.nguon_thong_tin_chung.Add("<DIA_BAN_VIET_HOA>");
                this.dich_thong_tin_chung.Add("THÀNH PHỐ HẢI DƯƠNG");

                this.nguon_thong_tin_chung.Add("<DIA_BAN_1>");
                this.dich_thong_tin_chung.Add("Hải Dương");
            }
            else
            {
                this.nguon_thong_tin_chung.Add("<DIA_BAN>");
                this.dich_thong_tin_chung.Add(chi_nhanh.ten_CN.Substring(19));

                this.nguon_thong_tin_chung.Add("<DIA_BAN_VIET_HOA>");
                this.dich_thong_tin_chung.Add(chi_nhanh.ten_CN.Substring(19).ToUpper());

                this.nguon_thong_tin_chung.Add("<DIA_BAN_1>");
                this.dich_thong_tin_chung.Add(chi_nhanh.ten_CN.Substring(25));
            }
            ...
            {
                this.nguon_thong_tin_chung.Add("<GNN_TEN_DIEM_GIAO_DICH>");
                this.dich_thong_tin_chung.Add(chi_nhanh.ten_CN);

                this.nguon_thong_tin_chung.Add("<GNN_TEN_PHONG>");
                this.dich_thong_tin_chung.Add("KHDN");
            }
            else if (can_bo_tin_dung.ma_pb == "2300-KHHSXCN")
            {
                this.nguon_thong_tin_chung.Add("<GNN_TEN_PHONG>");
                this.dich_thong_tin_chung.Add("KH HSX&CN");
            }
            else
            {
                this.nguon_thong_tin_chung.Add("<GNN_TEN_PHONG>");
                this.dich_thong_tin_chung.Add("KH&KD");
            }

            //Lấy tên điểm giao dịch
            Phonggiaodich phong_giao_dich = PhonggiaodichBUS.PGD_THEO_MA(can_bo_tin_dung.ma_pgd);
            ...
            this.nguon_thong_tin_chung.Add("<HDV_NGAY_HD_VAY>");
            this.dich_thong_tin_chung.Add(hd.ngay_hd_vay);
            ...
            //}
            //----------------------------------------------------------------------------------------------------
            //----------------------Khai báo tên khách hàng, số chứng minh thư, ngày cấp, nơi cấp trong đề nghị thế chấp tài sản bảo đảm và giấy nhận nợ-------------
            if (!string.IsNullOrEmpty(khv.hgd_ten_chong) && string.IsNullOrEmpty(khv.cn_ten) && string.IsNullOrEmpty(khv.tc_ten))
            {
                this.nguon_thong_tin_chung.Add("<GNN_KHV>");
                this.dich_thong_tin_chung.Add("ông "+khv.hgd_ten_chong+", bà "+khv.hgd_ten_vo);
                ...
                if (khv.hgd_dai_dien == "Chồng")
                {
                    ...
                    this.nguon_thong_tin_chung.Add("<KHV_CMT>");
                    this.dich_thong_tin_chung.Add(khv.hgd_cmnd_chong);

                    this.nguon_thong_tin_chung.Add("<KHV_NGAY_CAP_CMT>");
                    this.dich_thong_tin_chung.Add(khv.hgd_ngay_cap_cmnd_chong);

                    this.nguon_thong_tin_chung.Add("<KHV_NOI_CAP_CMT>");
                    this.dich_thong_tin_chung.Add(khv.hgd_noi_cap_cmnd_chong);

                    this.nguon_thong_tin_chung.Add("<KHV_DIA_CHI>");
                    this.dich_thong_tin_chung.Add(khv.hgd_dc_chong);
                }
                else
                {
            ...
            {
                this.nguon_thong_tin_chung.Add("<GNN_KHV>");
                this.dich_thong_tin_chung.Add("ông " + khv.cn_ten);
        ...
        #region LAY_THONG_TIN_GIAY_NHAN_NO
        //Lấy thông tin giấy nhận nợ
        internal void Lay_thong_tin_gnn(int id_giay_nhan_no)
        {
            this.nguon_gnn.Clear();
            this.dich_gnn.Clear();

            Giaynhanno gnn = AGRIBANKHD.BUS.GiaynhannoBUS.GIAY_NHAN_NO_THEO_ID(id_giay_nhan_no);

            this.nguon_gnn.Add("<GNN_DU_NO_TRUOC>");
            this.dich_gnn.Add(ControlFormat.ToFormatPrice(gnn.du_no_truoc));

            this.nguon_gnn.Add("<GNN_DU_NO_TRUOC_BANG_CHU>");
            this.dich_gnn.Add(CommonMethods.FirstCharToUpper(CommonMethods.ChuyenSoSangChu(Convert.ToString(gnn.du_no_truoc))));

            this.nguon_gnn.Add("<GNN_SO_TIEN_NHAN_NO>");
            this.dich_gnn.Add(ControlFormat.ToFormatPrice(gnn.so_tien_nhan_no));

            this.nguon_gnn.Add("<GNN_SO_TIEN_NHAN_NO_BANG_CHU>");
            this.dich_gnn.Add(CommonMethods.FirstCharToUpper(CommonMethods.ChuyenSoSangChu(Convert.ToString(gnn.so_tien_nhan_no))));

            this.nguon_gnn.Add("<GNN_DU_NO_HIEN_TAI>");
            this.dich_gnn.Add(ControlFormat.ToFormatPrice(gnn.du_no_hien_tai));

            this.nguon_gnn.Add("<GNN_DU_NO_HIEN_TAI_BANG_CHU>");
            this.dich_gnn.Add(CommonMethods.FirstCharToUpper(CommonMethods.ChuyenSoSangChu(Convert.ToString(gnn.du_no_hien_tai))));

            this.nguon_gnn.Add("<GNN_NGUOI_THU_HUONG_1>");
            this.dich_gnn.Add(gnn.nguoi_thu_huong_1);

            this.nguon_gnn.Add("<GNN_SO_TAI_KHOAN_1>");
            this.dich_gnn.Add(gnn.so_tai_khoan_1);

            this.nguon_gnn.Add("<GNN_NGAN_HANG_1>");
            this.dich_gnn.Add(gnn.ngan_hang_1);

            this.nguon_gnn.Add("<GNN_SO_TIEN_NHAN_NO_1>");
            this.dich_gnn.Add(ControlFormat.ToFormatPrice(gnn.so_tien_nhan_no_1));

            this.nguon_gnn.Add("<GNN_NGUOI_THU_HUONG_2>");
            this.dich_gnn.Add(gnn.nguoi_thu_huong_2);

            this.nguon_gnn.Add("<GNN_SO_TAI_KHOAN_2>");
            this.dich_gnn.Add(gnn.so_tai_khoan_2);

            this.nguon_gnn.Add("<GNN_NGAN_HANG_2>");
            this.dich_gnn.Add(gnn.ngan_hang_2);

            this.nguon_gnn.Add("<GNN_SO_TIEN_NHAN_NO_2>");
            this.dich_gnn.Add(ControlFormat.ToFormatPrice(gnn.so_tien_nhan_no_2));

            this.nguon_gnn.Add("<GNN_NGUOI_THU_HUONG_3>");
            this.dich_gnn.Add(gnn.nguoi_thu_huong_3);

            this.nguon_gnn.Add("<GNN_SO_TAI_KHOAN_3>");
            this.dich_gnn.Add(gnn.so_tai_khoan_3);

            this.nguon_gnn.Add("<GNN_NGAN_HANG_3>");
            this.dich_gnn.Add(gnn.ngan_hang_3);

            this.nguon_gnn.Add("<GNN_SO_TIEN_NHAN_NO_3>");
            this.dich_gnn.Add(ControlFormat.ToFormatPrice(gnn.so_tien_nhan_no_3));

            this.nguon_gnn.Add("<GNN_HAN_TRO_NO_GOC>");
            this.dich_gnn.Add(gnn.han_tra_no_goc);

            this.nguon_gnn.Add("<GNN_SO_GIAY_NHAN_NO>");
            this.dich_gnn.Add(gnn.so_giay_nhan_no);

            this.nguon_gnn.Add("<GNN_MUC_DICH_SU_DUNG_TIEN_VAY>");
            this.dich_gnn.Add(gnn.muc_dich_su_dung_tien_vay);

            this.nguon_gnn.Add("<CHUNG_TU_GIAI_NGAN>");
            this.dich_gnn.Add(gnn.chung_tu_giai_ngan);

            this.nguon_gnn.Add("<GNN_NGAY_NHAN_NO>");
            this.dich_gnn.Add(gnn.ngay_nhan_no);
        }
        //Kết thúc lấy thông tin giấy nhận nợ
        #endregion  
        ...
        private void dgvGiay_nhan_no_SelectionChanged(object sender, EventArgs e)
        {
            if (dgvGiay_nhan_no.Rows.Count == 0)
            {
                btnCap_nhat.Enabled = false;
                txtNgay_nhan_no.Text = DateTime.Now.ToString("dd/MM/yyyy");
                txtNgay_de_xuat_giai_ngan.Text = DateTime.Now.ToString("dd/MM/yyyy");
            ...
            {
                btnCap_nhat.Enabled = true;
                int selectedrowindex = dgvGiay_nhan_no.CurrentCell.RowIndex;
                //int selectedrowindex = dgvDanh_sach_tstc_1.SelectedCells[0].RowIndex;
                DataGridViewRow selectedRow = dgvGiay_nhan_no.Rows[selectedrowindex];
                id_giay_nhan_no = Convert.ToInt32(selectedRow.Cells["ID_GIAY_NHAN_NO"].Value);
                Giaynhanno gnn = AGRIBANKHD.BUS.GiaynhannoBUS.GIAY_NHAN_NO_THEO_ID(id_giay_nhan_no);
                txtNgay_nhan_no.Text = gnn.ngay_nhan_no;
                txtNgay_de_xuat_giai_ngan.Text = gnn.ngay_de_xuat_giai_ngan;
                txtDu_no_truoc.Text = Convert.ToString(gnn.du_no_truoc);
                txtSo_tien_nhan_no.Text = Convert.ToString(gnn.so_tien_nhan_no);
                txtDu_no_hien_tai.Text = Convert.ToString(gnn.du_no_hien_tai);
                txtNguoi_thu_huong_1.Text = gnn.nguoi_thu_huong_1;
                txtSo_tai_khoan_1.Text = gnn.so_tai_khoan_1;
                txtNgan_hang_1.Text = gnn.ngan_hang_1;
                txtSo_tien_nhan_no_1.Text = Convert.ToString(gnn.so_tien_nhan_no_1);
                txtNguoi_thu_huong_2.Text = gnn.nguoi_thu_huong_2;
                txtSo_tai_khoan_2.Text = gnn.so_tai_khoan_2;
                txtNgan_hang_2.Text = gnn.ngan_hang_2;
                txtSo_tien_nhan_no_2.Text = Convert.ToString(gnn.so_tien_nhan_no_2);
                txtNguoi_thu_huong_3.Text = gnn.nguoi_thu_huong_3;
                txtSo_tai_khoan_3.Text = gnn.so_tai_khoan_3;
                txtNgan_hang_3.Text = gnn.ngan_hang_3;
                txtSo_tien_nhan_no_3.Text = Convert.ToString(gnn.so_tien_nhan_no_3);
                txtSo_giay_nhan_no.Text = gnn.so_giay_nhan_no;
                txtHan_tro_no_goc.Text = gnn.han_tra_no_goc;
                txtMuc_dich_su_dung_von_vay.Text = gnn.muc_dich_su_dung_tien_vay;

                if (gnn.chung_tu_giai_ngan == "Hóa đơn mua hàng")
                {
                    rbHoa_don_mua_hang.Checked = true;
                    rbKhac.Checked = false;
                    txtChung_tu_giai_ngan_khac.Enabled = false;
                ...
                {
                    rbHoa_don_mua_hang.Checked = false;
                    rbKhac.Checked = true;
                    txtChung_tu_giai_ngan_khac.Enabled = true;
                    txtChung_tu_giai_ngan_khac.Text = gnn.chung_tu_giai_ngan;
                }
            }
        }

        //Định dạng số cho các ô số tiền
        private void txtDu_no_truoc_TextChanged(object sender, EventArgs e)
        {
            if (txtDu_no_truoc.Text == "")
            {
                txtDu_no_truoc.Text = null;
            }
            else
            {
                if (txtDu_no_truoc.Text != "")
                {
                    Int64 d = Convert.ToInt64(ControlFormat.skipComma(txtDu_no_truoc.Text));
                    txtDu_no_truoc.Text = d.ToString("#,#", System.Globalization.CultureInfo.InvariantCulture);
                }
            }
            txtDu_no_truoc.Select(txtDu_no_truoc.Text.Length, 0);
            
        }

        private void txtSo_tien_nhan_no_TextChanged(object sender, EventArgs e)
        {
            if (txtSo_tien_nhan_no.Text == "")
            {
                txtSo_tien_nhan_no.Text = null;
            }
            else
            {
                if (txtSo_tien_nhan_no.Text != "")
                {
                    Int64 d = Convert.ToInt64(ControlFormat.skipComma(txtSo_tien_nhan_no.Text));
                    txtSo_tien_nhan_no.Text = d.ToString("#,#", System.Globalization.CultureInfo.InvariantCulture);
                }
            }
            txtSo_tien_nhan_no.Select(txtSo_tien_nhan_no.Text.Length, 0);
        }

        private void txtDu_no_hien_tai_TextChanged(object sender, EventArgs e)
        {
            if (txtDu_no_hien_tai.Text == "")
            {
                txtDu_no_hien_tai.Text = null;
            }
            else
            {
                if (txtDu_no_hien_tai.Text != "")
                {
                    Int64 d = Convert.ToInt64(ControlFormat.skipComma(txtDu_no_hien_tai.Text));
                    txtDu_no_hien_tai.Text = d.ToString("#,#", System.Globalization.CultureInfo.InvariantCulture);
                }
            }
            txtDu_no_hien_tai.Select(txtDu_no_hien_tai.Text.Length, 0);
        }

        private void txtSo_tien_nhan_no_1_TextChanged(object sender, EventArgs e)
        {
            if (txtSo_tien_nhan_no_1.Text == "")
            {
                txtSo_tien_nhan_no_1.Text = null;
            }
            else
            {
                if (txtSo_tien_nhan_no_1.Text != "")
                {
                    Int64 d = Convert.ToInt64(ControlFormat.skipComma(txtSo_tien_nhan_no_1.Text));
                    txtSo_tien_nhan_no_1.Text = d.ToString("#,#", System.Globalization.CultureInfo.InvariantCulture);
                }
            }
            txtSo_tien_nhan_no_1.Select(txtSo_tien_nhan_no_1.Text.Length, 0);

            Int64 So_tien_nhan_no_1;
            if (string.IsNullOrEmpty(txtSo_tien_nhan_no_1.Text))
            {
                So_tien_nhan_no_1 = 0;
            }
            else
            {
                So_tien_nhan_no_1 = Convert.ToInt64(ControlFormat.skipComma(txtSo_tien_nhan_no_1.Text));
            }

            Int64 So_tien_nhan_no_2;
            if (string.IsNullOrEmpty(txtSo_tien_nhan_no_2.Text))
            {
                So_tien_nhan_no_2 = 0;
            }
            else
            {
                So_tien_nhan_no_2 = Convert.ToInt64(ControlFormat.skipComma(txtSo_tien_nhan_no_2.Text));
            }

            Int64 So_tien_nhan_no_3;
            if (string.IsNullOrEmpty(txtSo_tien_nhan_no_3.Text))
            {
                So_tien_nhan_no_3 = 0;
            }
            else
            {
                So_tien_nhan_no_3 = Convert.ToInt64(ControlFormat.skipComma(txtSo_tien_nhan_no_3.Text));
            }

            txtSo_tien_nhan_no.Text = Convert.ToString(So_tien_nhan_no_1 + So_tien_nhan_no_2 + So_tien_nhan_no_3);

            Int64 Du_no_truoc;
            if (string.IsNullOrEmpty(txtDu_no_truoc.Text))
            {
                Du_no_truoc = 0;
            }
            else
            {
                Du_no_truoc = Convert.ToInt64(ControlFormat.skipComma(txtDu_no_truoc.Text));
            }
            txtDu_no_hien_tai.Text = Convert.ToString(Du_no_truoc + So_tien_nhan_no_1 + So_tien_nhan_no_2 + So_tien_nhan_no_3);
        }

        private void txtSo_tien_nhan_no_2_TextChanged(object sender, EventArgs e)
        {
            if (txtSo_tien_nhan_no_2.Text == "")
            {
                txtSo_tien_nhan_no_2.Text = null;
            }
            else
            {
                if (txtSo_tien_nhan_no_2.Text != "")
                {
                    Int64 d = Convert.ToInt64(ControlFormat.skipComma(txtSo_tien_nhan_no_2.Text));
                    txtSo_tien_nhan_no_2.Text = d.ToString("#,#", System.Globalization.CultureInfo.InvariantCulture);
                }
            }
            txtSo_tien_nhan_no_2.Select(txtSo_tien_nhan_no_2.Text.Length, 0);
        ...
        private void txtSo_tien_nhan_no_3_TextChanged(object sender, EventArgs e)
        {
            if (txtSo_tien_nhan_no_3.Text == "")
            {
                txtSo_tien_nhan_no_3.Text = null;
            }
            else
            {
                if (txtSo_tien_nhan_no_3.Text != "")
                {
                    Int64 d = Convert.ToInt64(ControlFormat.skipComma(txtSo_tien_nhan_no_3.Text));
                    txtSo_tien_nhan_no_3.Text = d.ToString("#,#", System.Globalization.CultureInfo.InvariantCulture);
                }
            }
            txtSo_tien_nhan_no_3.Select(txtSo_tien_nhan_no_3.Text.Length, 0);
        ...
        {
            //Kiểm tra dữ liệu nhập hạn trả nợ gốc
            if (txtHan_tro_no_goc.Text == "  /  /")
            {
                MessageBox.Show("Bạn chưa nhập hạn trả nợ gốc. Đề nghị nhập lại!");
                txtHan_tro_no_goc.Focus();
                return;
            }
            else
            {
                if (!Utilities.CommonMethods.KiemTraNhapNgay(txtHan_tro_no_goc.Text))
                {
            ...
            //Kiểm tra dữ liệu nhập ngày nhận nợ
            if (txtNgay_nhan_no.Text == "  /  /")
            {
                MessageBox.Show("Bạn chưa nhập ngày nhận nợ. Đề nghị nhập lại!");
                txtNgay_nhan_no.Focus();
                return;
            }
            else
            {
                if (!Utilities.CommonMethods.KiemTraNhapNgay(txtNgay_nhan_no.Text))
                {
                    MessageBox.Show("Dữ liệu ngày nhận nợ chưa đúng định dạng dd/mm/yyyy, đề nghị nhập lại!");
                    txtNgay_nhan_no.Focus();
                    return;
                }
            }

            //Kiểm tra dữ liệu nhập ngày đề xuất giải ngân
            if (txtNgay_de_xuat_giai_ngan.Text == "  /  /")
            {
                MessageBox.Show("Bạn chưa nhập ngày báo cáo đề xuất giải ngân. Đề nghị nhập lại!");
                txtNgay_de_xuat_giai_ngan.Focus();
                return;
            }
            else
            {
                if (!Utilities.CommonMethods.KiemTraNhapNgay(txtNgay_de_xuat_giai_ngan.Text))
                {
                    MessageBox.Show("Dữ liệu ngày báo cáo đề xuất giải ngân chưa đúng định dạng dd/mm/yyyy, đề nghị nhập lại!");
                    txtNgay_de_xuat_giai_ngan.Focus();
                    return;
                }
            }

            string[] giay_nhan_no = new string[23];
            giay_nhan_no[1] = txtMa_hd_vay.Text;
            giay_nhan_no[2] = txtNgay_nhan_no.Text;
            if (!string.IsNullOrEmpty(txtDu_no_truoc.Text))
            {
                giay_nhan_no[3] = ControlFormat.skipComma(txtDu_no_truoc.Text);
            }
            else
            {
                giay_nhan_no[3] = "0";
            ...
            if (!string.IsNullOrEmpty(txtSo_tien_nhan_no.Text))
            {
                giay_nhan_no[4] = ControlFormat.skipComma(txtSo_tien_nhan_no.Text);
            }
            else
            {
                giay_nhan_no[4] = "0";
            ...
            if (!string.IsNullOrEmpty(txtDu_no_hien_tai.Text))
            {
                giay_nhan_no[5] = ControlFormat.skipComma(txtDu_no_hien_tai.Text);
            }
            else
            {
                giay_nhan_no[5] = "0";
            ...
            giay_nhan_no[6] = txtNguoi_thu_huong_1.Text;
            giay_nhan_no[7] = txtSo_tai_khoan_1.Text;
            giay_nhan_no[8] = txtNgan_hang_1.Text;

            if (!string.IsNullOrEmpty(txtSo_tien_nhan_no_1.Text))
            {
                giay_nhan_no[9] = ControlFormat.skipComma(txtSo_tien_nhan_no_1.Text);
            }
            else
            {
                giay_nhan_no[9] = "0";
            ...
            giay_nhan_no[10] = txtNguoi_thu_huong_2.Text;
            giay_nhan_no[11] = txtSo_tai_khoan_2.Text;
            giay_nhan_no[12] = txtNgan_hang_2.Text;

            if (!string.IsNullOrEmpty(txtSo_tien_nhan_no_2.Text))
            {
                giay_nhan_no[13] = ControlFormat.skipComma(txtSo_tien_nhan_no_2.Text);
            }
            else
            {
                giay_nhan_no[13] = "0";
            ...
            giay_nhan_no[14] = txtNguoi_thu_huong_3.Text;
            giay_nhan_no[15] = txtSo_tai_khoan_3.Text;
            giay_nhan_no[16] = txtNgan_hang_3.Text;

            if (!string.IsNullOrEmpty(txtSo_tien_nhan_no_3.Text))
            {
                giay_nhan_no[17] = ControlFormat.skipComma(txtSo_tien_nhan_no_3.Text);
            }
            else
            {
                giay_nhan_no[17] = "0";
            ...
            giay_nhan_no[18] = txtSo_giay_nhan_no.Text;
            giay_nhan_no[19] = txtHan_tro_no_goc.Text;
            giay_nhan_no[20] = txtMuc_dich_su_dung_von_vay.Text;

            if (rbHoa_don_mua_hang.Checked == true)
            {
                giay_nhan_no[21] = "Hóa đơn mua hàng";
            }
            else
            {
                giay_nhan_no[21] = txtChung_tu_giai_ngan_khac.Text;
            }

            giay_nhan_no[22] = txtNgay_de_xuat_giai_ngan.Text;

            if (tong_han_muc_tin_dung < Convert.ToInt64(ControlFormat.skipComma(txtDu_no_hien_tai.Text)))
            {
                MessageBox.Show("Tổng dư nợ sau nhận nợ vượt quá hạn mức. Đề nghị nhập lại số tiền nhận nợ!");
                return;
            }
            else
            {
                Giaynhanno gnn = new Giaynhanno(giay_nhan_no);
                if (gnn_bus.THEM_GIAY_NHAN_NO(gnn))
                {
                    MessageBox.Show("Thêm giấy nhận nợ thành công!");
                    //this.Close();
                    this.Danh_sach_giay_nhan_no_theo_ma_hd_vay();
                }
                else MessageBox.Show("Có lỗi xảy ra!");
            }
        }

        private void txtDu_no_truoc_KeyPress(object sender, KeyPressEventArgs e)
        {
        ...
        private void txtSo_tien_nhan_no_KeyPress(object sender, KeyPressEventArgs e)
        {
            if ((e.KeyChar < 48 || e.KeyChar > 57) && e.KeyChar != 8)
            {
                e.Handled = true;
        ...
        private void txtDu_no_hien_tai_KeyPress(object sender, KeyPressEventArgs e)
        {
            if ((e.KeyChar < 48 || e.KeyChar > 57) && e.KeyChar != 8)
            {
                e.Handled = true;
        ...
        private void txtSo_tien_nhan_no_1_KeyPress(object sender, KeyPressEventArgs e)
        {
            if ((e.KeyChar < 48 || e.KeyChar > 57) && e.KeyChar != 8)
            {
                e.Handled = true;
        ...
        private void txtSo_tien_nhan_no_2_KeyPress(object sender, KeyPressEventArgs e)
        {
            if ((e.KeyChar < 48 || e.KeyChar > 57) && e.KeyChar != 8)
            {
                e.Handled = true;
        ...
        private void txtSo_tien_nhan_no_3_KeyPress(object sender, KeyPressEventArgs e)
        {
            if ((e.KeyChar < 48 || e.KeyChar > 57) && e.KeyChar != 8)
            {
                e.Handled = true;
        ...
        private void btnCap_nhat_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                if (!Utilities.CommonMethods.KiemTraNhapNgay(txtHan_tro_no_goc.Text))
                {
                    MessageBox.Show("Dữ liệu hạn trả nợ gốc chưa đúng định dạng dd/mm/yyyy, đề nghị nhập lại!");
                    txtHan_tro_no_goc.Focus();
                    return;
                }
            }
            ...
            string[] giay_nhan_no = new string[23];
            giay_nhan_no[0] = Convert.ToString(id_giay_nhan_no);
            giay_nhan_no[1] = txtMa_hd_vay.Text;
            giay_nhan_no[2] = txtNgay_nhan_no.Text;
            if (!string.IsNullOrEmpty(txtDu_no_truoc.Text))
            {
            ...
            {
                Giaynhanno gnn = new Giaynhanno(giay_nhan_no);
                if (gnn_bus.CAP_NHAT_GIAY_NHAN_NO(gnn))
                {
                    MessageBox.Show("Cập nhật giấy nhận nợ thành công!");
                    //this.Close();
                    this.Danh_sach_giay_nhan_no_theo_ma_hd_vay();
                }
                else MessageBox.Show("Có lỗi xảy ra!");
        ...
        private void btnXoa_Click(object sender, EventArgs e)
        {
            ...
            DataGridViewRow selectedRow = dgvGiay_nhan_no.Rows[selectedrowindex];
            id_giay_nhan_no = Convert.ToInt32(selectedRow.Cells["ID_GIAY_NHAN_NO"].Value);
            DialogResult dialogResult = MessageBox.Show("Bạn có chắc muốn xóa giấy nhận nợ này?", "Thông báo", MessageBoxButtons.YesNo);
            if (dialogResult == DialogResult.Yes)
            {
                dgvGiay_nhan_no.DataSource = null;
                dgvGiay_nhan_no.Rows.Clear();
                if (gnn_bus.XOA_GIAY_NHAN_NO(id_giay_nhan_no))
                {
                    MessageBox.Show("Xóa giấy nhận nợ thành công!");
                    //this.Close();
                    this.Danh_sach_giay_nhan_no_theo_ma_hd_vay();
                }
                else MessageBox.Show("Có lỗi xảy ra!");
        ...
        #region Tao_giay_nhan_no
        //-------------------------------------TẠO GIẤY NHẬN NỢ --------------------------------------------------------------

        internal void GIAY_NHAN_NO(string giay_nhan_no)
        {
            savefileTao_giay_nhan_no.Filter = "Word Documents|*.docx";
            if (savefileTao_giay_nhan_no.ShowDialog() == DialogResult.OK)
            {
                string TemplateFileLocation = CommonMethods.TemplateFileLocation(@"GNN\" + giay_nhan_no + ".docx");
                //CommonMethods.CreateWordDocument(TemplateFileLocation, savefileTao_giay_nhan_no.FileName, this.nguon_thong_tin_chung, this.dich_thong_tin_chung);
                CommonMethods.CreateWordDocument2(TemplateFileLocation, savefileTao_giay_nhan_no.FileName, this.nguon_thong_tin_chung, this.dich_thong_tin_chung, this.nguon_gnn, this.dich_gnn);
                MessageBox.Show("File đã được tạo tại đường dẫn: " + savefileTao_giay_nhan_no.FileName, "Tạo file thành công");
            }
        }

        private void btnTao_giay_nhan_no_Click(object sender, EventArgs e)
        {
            IFormatProvider culture = new System.Globalization.CultureInfo("fr-FR", true);
            DateTime dt = DateTime.Parse(txtNgay_nhan_no.Text, culture, System.Globalization.DateTimeStyles.AssumeLocal);
            this.nguon_thong_tin_chung.Add("<GNN_NGAY>");
            this.dich_thong_tin_chung.Add(Convert.ToString(dt.Day));

            this.nguon_thong_tin_chung.Add("<GNN_THANG>");
            this.dich_thong_tin_chung.Add(Convert.ToString(dt.Month));

            this.nguon_thong_tin_chung.Add("<GNN_NAM>");
            this.dich_thong_tin_chung.Add(Convert.ToString(dt.Year));

            int selectedrowindex = dgvGiay_nhan_no.CurrentCell.RowIndex;
            DataGridViewRow selectedRow = dgvGiay_nhan_no.Rows[selectedrowindex];
            id_giay_nhan_no = Convert.ToInt32(selectedRow.Cells["ID_GIAY_NHAN_NO"].Value);
            this.Lay_thong_tin_gnn(id_giay_nhan_no);

            this.GIAY_NHAN_NO("GIAY_NHAN_NO");
        }
        #endregion
        //------------------------------------KẾT THÚC TẠO GIẤY NHẬN NỢ---------------

        //------------------------TẠO BÁO CÁO ĐỀ XUẤT GIẢI NGÂN------------------------------
        #region Tao_bao_cao_de_xuat_giai_ngan

        internal void BAO_CAO_DE_XUAT_GIAI_NGAN(string bao_cao_de_xuat_giai_ngan)
        {
            savefileTao_bao_cao_de_xuat_giai_ngan.Filter = "Word Documents|*.docx";
            if (savefileTao_bao_cao_de_xuat_giai_ngan.ShowDialog() == DialogResult.OK)
            {
                string TemplateFileLocation = CommonMethods.TemplateFileLocation(@"DXGN\" + bao_cao_de_xuat_giai_ngan + ".docx");
                //CommonMethods.CreateWordDocument(TemplateFileLocation, savefileTao_giay_nhan_no.FileName, this.nguon_thong_tin_chung, this.dich_thong_tin_chung);
                CommonMethods.CreateWordDocument2(TemplateFileLocation, savefileTao_bao_cao_de_xuat_giai_ngan.FileName, this.nguon_thong_tin_chung, this.dich_thong_tin_chung, this.nguon_gnn, this.dich_gnn);
                MessageBox.Show("File đã được tạo tại đường dẫn: " + savefileTao_bao_cao_de_xuat_giai_ngan.FileName, "Tạo file thành công");
            }
        }

        private void btnTao_bao_cao_de_xuat_giai_ngan_Click(object sender, EventArgs e)
        {
            IFormatProvider culture = new System.Globalization.CultureInfo("fr-FR", true);
            ...
            this.BAO_CAO_DE_XUAT_GIAI_NGAN("BAO_CAO_DE_XUAT_GIAI_NGAN");
        }
        #endregion
        ...
        //-------------------KẾT THÚC TẠO BÁO CÁO ĐỀ XUẤT GIẢI NGÂN---------------------------
